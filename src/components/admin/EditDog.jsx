...
const EditDog = () => {
  const { clientId, dogId } = useParams();
  ...
  useEffect(() => {
    if (isAdmin && clientId && dogId) {
      loadData();
    }
  }, [isAdmin, clientId, dogId]);

  const loadData = async () => {
    try {
      // Load client info
      const { data: clientData, error: clientError } = await db.getProfileWithUserData(clientId);
      ...
      // Load dog data
      const { data: dogData, error: dogError } = await db.supabase
        .from('dogs')
        .select('*')
        .eq('id', dogId)
      ...
      showMessage('Error loading dog information', 'danger');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const { error } = await db.updateDog(dogId, dogData);

      if (error) throw error;

      showMessage(`${formData.name} has been updated successfully!`, 'success');
      ...
      showMessage('Failed to update dog. Please try again.', 'danger');
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="max-w-4xl mx-auto py-8 px-4">
      ...
      <div className="mb-8">
        ...
        <p className="text-gray-600 mt-2">Editing dog for <strong>{clientName}</strong></p>
      </div>
